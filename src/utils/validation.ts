...
export const applicationValidationSchema = yup.object({
  ...
  client_secret: yup
    ...
    .min(2, "Client secret must be at least 2 characters")
    .max(50, "Client secret must not exceed 50 characters")
    .matches(
      /^[a-zA-Z0-9_-]+$/,
      "Client secret can only contain letters, numbers, hyphens, and underscores",
    ),
  version: yup
    .string()
    .required("Version is required")
    .min(2, "Version must be at least 2 characters")
...
export const accountValidationSchema = yup.object({
  ...
  accountType: yup
    .string()
    .oneOf(
      ["Temporary", "Personal", "Business"],
      "Account type must be Temporary, Personal, or Business",
    )
...
export const rightsValidationSchema = yup.object({
  ...
  permissions: yup
    ...
    .of(
      yup
        .string()
        .oneOf(["read", "write", "admin", "owner", "delete"])
        .required(),
    )
...
export const accountSharingValidationSchema = yup.object({
  ...
  invitedBy: yup.string().required("Invited by is required"),
  expiresAt: yup
    .string()
    .optional()
    .test(
...
export const registerValidationSchema = yup.object({
  ...
  role: yup.string().oneOf(["admin", "user", "manager"]).optional(),
});
